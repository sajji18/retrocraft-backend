...
const signup = async (req, res) => {
    ...
    if (role === 'FREELANCER') {
        ...
        if (freelancer) {
            ...
            const token = jwt.sign({ username, role: newFreelancer.role }, secretKey, { expiresIn: '1h' });
        ...
        if (producer) {
            ...
            const token = jwt.sign({ username, role: newProducer.role }, secretKey, { expiresIn: '1h' });
            res.status(201).json({ message: "User Account created successfully", role: role, token: token });
        }
    }
